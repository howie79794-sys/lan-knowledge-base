...
- 文件上传、列表、筛选、详情、下载。
- 两类核心分类：
  - 文件作用：招投标需求清单、用户详细需求清单、规划材料、政策法规、产品社区文档、业务知识、客户案例、业务材料。
  - 文件格式：由系统根据扩展名自动识别。
- PDF、Word、PPT、Excel 的基础文本抽取。
- 为 Agent 提供只读 API：manifest、列表筛选、正文读取、原文下载、OpenAPI。
- 操作日志：上传、下载、删除、重建索引。
- 每日备份数据库和文件目录。
...
- 顶部：关键词搜索、文件作用筛选、文件格式筛选、状态筛选。
- 主体：文件表格，列包括标题、文件作用、格式、大小、解析状态、更新时间、操作。
- 操作：详情、下载、复制 Agent 内容链接、重新解析。
...
上传成功后立即写入 `documents`，状态为 `uploaded`，再生成转换任务。

MVP 可以先用内置后台线程或定时轮询 worker，不必上 Redis：

1. 上传原文件到 `uploads/`。
2. 计算 sha256，落库。
3. 创建 `conversion_jobs`。
4. worker 读取 queued job。
5. 按文件格式调用解析器：
